    ...
    <>
      <Card>
        <Flex gap={2} alignItems="center" justifyContent="space-between">
          <Flex gap={2}>
            <Checkbox></Checkbox>
            <Heading size="sm" onClick={openModal}>
              {objective.title}
            </Heading>
          </Flex>
          <Flex gap={2}>
...
export const RoutineObjectives = ({ routine }: RoutineObjectivesProps) => {
  ...
  const deleteItem = (objectiveId: string) => {
    client
      .delete(`/routines/${routine.id}/${objectiveId}`)
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
  ...
  return (
    <Stack alignItems="flex-start" width="100%" color="brand.primaryText">
      ...
      <Stack gap={2} width="100%">
        ...
        {edit ? (
          <NewObjective closeEditor={closeEditor} onSave={onSave} />
        ) : (
          <Button
            size="sm"
            width="fit-content"
